 ...
 * @date 2020/05/12 11:52
 */
object AreaClickCountsStream {
  def main(args: Array[String]): Unit = {
    ...
    val areaCounts: DStream[(String, Int)] = cleanLog.map(line => {
      var area: String = ""
      if (line.area != "") {
        area = line.area
      }
    ...
    areaCounts.foreachRDD { rdd =>
      rdd.foreachPartition { partitionOfRecords =>
        ...
        try {
          ...
          // 写入到redis
          partitionOfRecords.foreach( record => jedis.hincrBy("areaCounts", record._1, record._2))
        } catch {
          case ex: Exception =>
            ex.printStackTrace()
        } finally {
